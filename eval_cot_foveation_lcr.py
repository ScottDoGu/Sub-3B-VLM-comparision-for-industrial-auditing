...
def main():
    runs = [1, 2, 3]
    results_dir = "results/innovation/foveation"
    ...
    f1s = []
    for run in runs:
        filename = f"qwen2_vl_foveation_run_{run}_results.csv"
    ...
    if lcrs:
        ...
        print(f"Agentic Foveation       | {mean_lcr*100:>5.1f}% ± {std_lcr*100:<4.1f} | {mean_f1*100:>5.1f}% ± {std_f1*100:<4.1f}")
        summary.append({
            "Configuration": "Agentic Foveation",
            "LCR_Mean": mean_lcr,
            "LCR_Std": std_lcr,
            "F1_Mean": mean_f1,
            "F1_Std": std_f1
    ...
    summary_df.to_csv(os.path.join(metrics_dir, "foveation_aggregated_metrics.csv"), index=False)
    print(f"\nAggregated results saved to {metrics_dir}/foveation_aggregated_metrics.csv")
