...
def build_stage3_prompt(constraint, category):
    """
    Build the Stage 3 reasoning prompt dynamically.
    Generalised for both gauge (numeric) and pipeline (qualitative) images.
    The constraint is injected verbatim — no hardcoded thresholds.
    """
    if category == "guage":
        return (
            "You are an industrial safety auditor inspecting field equipment. "
            "Examine this gauge image carefully.\n"
            "Note: Gauges may have multiple measurement scales (e.g., inner and outer rings "
            "with different units). Read the scale whose unit matches the safety rule.\n"
            f"Safety Rule: '{constraint}'\n\n"
            "Step 1: Identify all visible scales and their units.\n"
            "Step 2: Read the exact numerical value indicated by the needle on the relevant scale.\n"
            "Step 3: Apply the safety rule to that reading.\n"
            "Step 4: Provide a final verdict. Output exactly [SAFE] or [UNSAFE]."
        )
    else:
        # Pipeline / structural inspection
        return (
            "You are an industrial safety auditor inspecting structural equipment. "
            "Examine this close-up image carefully.\n"
            f"Safety Rule: '{constraint}'\n\n"
            "Step 1: Describe the visible physical condition of the surface "
            "(e.g., corrosion, cracks, leaks, clean metal).\n"
            "Step 2: Evaluate whether the observed condition violates the safety rule.\n"
            "Step 3: Provide a final verdict. Output exactly [SAFE] or [UNSAFE]."
        )
...
def main():
    ...
    for run_i in range(1, N_RUNS + 1):
        ...
        for i, item in enumerate(dataset):
            ...
            stage3_prompt = build_stage3_prompt(constraint, category)
        ...
        save_results(
            results,
            "qwen2_vl_cot_foveation",
            iteration=run_i,
            out_dir="results/innovation/cot_foveation",
        )
