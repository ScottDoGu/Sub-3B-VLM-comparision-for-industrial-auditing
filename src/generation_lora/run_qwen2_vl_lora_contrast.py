"""
Qwen2-VL-2B + LoRA Adapter + CLAHE + Rule Decomposition (Full Stack)
The ceiling configuration: LoRA fine-tuned vision encoder with both
CLAHE preprocessing and structured decomposition prompting.
"""
import torch
...
for run_i in range(1, N_RUNS + 1):
    ...
    print(f"\n--- Starting LoRA+CLAHE+Decomp Run {run_i}/{N_RUNS} on {len(dataset)} images ---")
    ...
    for i, item in enumerate(dataset):
        ...
        # Rule Decomposition Prompt (identical to generation_contrast)
        decomp_prompt = (
    ...
    save_results(results, "qwen2_vl_lora_contrast", iteration=run_i, out_dir="results/innovation/lora")
