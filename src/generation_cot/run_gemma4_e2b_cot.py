...
for run_i in range(1, N_RUNS + 1):
    ...
    for i, item in enumerate(dataset):
        ...
        cot_prompt = (
            ...
            f"Identify if the image violates the rule. Format your answer EXACTLY as follows:\n\n"
            f"STEP 1 - OBSERVATION: [Detail exactly what you see regarding the rule]\n"
            f"STEP 2 - RULE APPLICATION: [Explain how the observation violates or complies with the rule]\n"
            f"STEP 3 - VERDICT: [SAFE or UNSAFE]\n"
        )
        ...
        with torch.no_grad():
            generated_ids = model.generate(
                **inputs, 
                max_new_tokens=384,
                do_sample=True,
                temperature=0.2, # slight temp to allow reasoning divergence
    ...
    # 5. Save Results
    save_results(results, "gemma4_e2b_cot", iteration=run_i, out_dir="results/innovation/cot")
